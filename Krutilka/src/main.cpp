...
//регистры предыдущих значений семафоров (на предыдущем шаге цикла)
...
#define _time1 70
#define _time2 170
//константы отсечек значений считанных с аналоговых входов
#define minval 270
#define maxval 415
...
#endif
//
//таймер срабатывает примерно раз в 10 микросекунд
//ведется обратный отсчет по таймерам колес истечение таймера, значение ==0, означает превышение периода 
//отсчитанного с момента наступление события реинициализации счетчика таймера и тогда мы обрабатываем превышение периода
...
//если ппроизошел переход включения магнита на притяжение начинаем отсчет занова
  if ( _IN1_value == LOW && IN1_value == HIGH ) timer1 = time1;
//если произошло переключение магнита на отталкивание проверяем значения счетчиков
...
// --------------------------------------------------------------------------------------
// сохраяем семафоры направления магнитов в регистры предыдущих состояний
